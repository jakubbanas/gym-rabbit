<script lang="ts">
	...
	function toggleSet(
		...
		const currentValue = state.progress[currentPlan.id]?.[exerciseId]?.[setIndex]?.[type] ?? false;
		state = updateProgress(currentPlan.id, exerciseId, setIndex, type, !currentValue);
	}

	function isSetChecked(exerciseId: string, setIndex: number, type: 'left' | 'right' | 'completed'): boolean {
		if (!currentPlan) return false;
		return state.progress[currentPlan.id]?.[exerciseId]?.[setIndex]?.[type] ?? false;
	}

	function getCompletedSets(exercise: Exercise): number {
		if (!currentPlan) return 0;
		
		const exerciseProgress = state.progress[currentPlan.id]?.[exercise.id] ?? {};
		let completed = 0;
											...
											<div class="checkbox-group">
												<label class="checkbox-label">
													<input
														...
														onchange={() => toggleSet(exercise.id, setIndex, 'left')}
													/>
													<span>L</span>
												</label>
												<label class="checkbox-label">
													<input
														...
														onchange={() => toggleSet(exercise.id, setIndex, 'right')}
													/>
													<span>R</span>
												</label>
											</div>
										{:else}
											<label class="checkbox-label">
												<input
													...
													onchange={() => toggleSet(exercise.id, setIndex, 'completed')}
												/>
												<span>✓</span>
											</label>
										{/if}
